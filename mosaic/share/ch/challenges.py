...
import sys
sys.path.insert(0, '../')
from Global import PATH_CHALLENGES
...
def load_challenge_data(name):
    f=h5py.File(PATH_CHALLENGES + name+".h5","r")
...
def load_challenge(name,train_prop=0.9,batch_size=64):
    ...
    #FIXME: normalisation should not be called, random points do not explore the full range of the accessible interval
    #x = normalise(x, [-1,1])
    #y = normalise(y, [-1,1])
    #x = normalise(x, [-np.pi,np.pi])
    #y = normalise(y, [-np.pi,np.pi])
...
def normalise(bounds, vals, Vals):
    ...
    #assert([type(v)==list for v in vals])
